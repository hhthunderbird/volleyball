...
public class BallTrajectory : MonoBehaviour
{
    ...
    private float _kickAngle = 45f; // Angle of kick in degrees
    private float _ballMass = 1f; // Mass of the ball
    ...
    void CalculateTrajectory()
    {
        ...
        float angleRad = _kickAngle * Mathf.Deg2Rad;

        // Calculate initial velocity components
        float v0 = _kickForce / _ballMass; // v = F/m
        float v0x = v0 * Mathf.Cos( angleRad );
        float v0y = v0 * Mathf.Sin( angleRad );

        // Calculate time of flight
        float discriminant = Mathf.Sqrt( Mathf.Pow( v0y, 2 ) + 2 * gravity * _objectHeight;
