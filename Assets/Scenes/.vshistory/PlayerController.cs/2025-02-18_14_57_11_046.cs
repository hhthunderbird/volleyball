...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ( Input.GetKey( KeyCode.A ) )
            _player1RB.MovePosition( _player1RB.position + Vector3.right* Time.deltaTime * _speed );
        if ( Input.GetKey( KeyCode.S ) )
            _player1RB.MovePosition( _player1RB.position + Vector3.back * Time.deltaTime * _speed );
        if ( Input.GetKey( KeyCode.D ) )
            _player1RB.MovePosition( _player1RB.position + Vector3.left * Time.deltaTime * _speed );
