...
public class PlayerController : NetworkBehaviour
{
    ...
    void Update()
    {
        if ( !IsOwner ) return; // Only the owning client should process inputs
    ...
    void FixedUpdate()
    {
        if ( !IsOwner ) return; // Only the owning client should send movement requests
    ...
    // ?? ServerRPC: The client sends movement requests to the server
    [ServerRpc]
