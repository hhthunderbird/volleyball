...
public class LobbyScreen : NetworkBehaviour
{
    ...
    private void Start()
    {
        ...
        _playerName = PlayerPrefs.GetString( PlayerPrefsKeys.PlayerName.ToString() );
        var side = PlayerPrefs.GetString( PlayerPrefsKeys.PlayerSide.ToString() );
        _playerSide = ( Side ) Enum.Parse( typeof( Side ), side );

        UpdateHostPlayerListRpc( NetworkManager.Singleton.LocalClientId, _playerName, _playerSide );
    }
