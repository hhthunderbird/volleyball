    ...
    private void Do()
    {
        ...
        for ( int h = 0; h < _height; h++ )
        {
            for ( int x = 0; x < _length; x++ )
            {
                ...
                var position = new Vector3( x * _sideSize, h * _sideSize, z );
                bgPiece.position = piece.transform.position = position;

                piece.OriginalLocation = new Vector3( x * _sideSize, h * _sideSize, z );
                ...
                var offset = new Vector2( x * horizontalTiling, h * verticalTiling );
