...
public class GridManager : MonoBehaviour
{
    ...
    private void ApplySprite( int x, int y, Piece piece )
    {
        ...
        //pBlock.SetTexture( "_MainTex", _mainImage );
        var renderer = piece.Renderer;

        var rect = new Rect( 0, 0, _mainImage.width, _mainImage.height );

        renderer.sprite = Sprite.Create( _mainImage, rect, Vector2.zero );
        ...
        if ( x == 0 && y == 0 )
        {
            var maskRect = new Rect( 0, 0, _topLeftCorner.width, _topLeftCorner.height );

            piece.Mask.sprite = Sprite.Create( _topLeftCorner, rect, Vector2.zero );
        }
