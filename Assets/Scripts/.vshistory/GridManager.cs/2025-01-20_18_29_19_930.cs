...
public class GridManager : MonoBehaviour
{
    ...
    private IEnumerator Do()
    {
        ...
        {
            _xScale = _mainImage.texelSize.x / _mainImage.texelSize.y;
        }
        else if ( _mainImage.texelSize.x < _mainImage.texelSize.y )
        {
            _yScale = _mainImage.texelSize.y / _mainImage.texelSize.x;
        }
        ...
        for ( int y = 0; y < _height; y++ )
        {
            for ( int x = 0; x < _length; x++ )
            {
                ...
                var position = new Vector3( x * _xScale, y * _yScale, z );
                bgPiece.position = piece.transform.position = position;

                piece.OriginalLocation = new Vector3( x * _xScale, y * _yScale, z );
