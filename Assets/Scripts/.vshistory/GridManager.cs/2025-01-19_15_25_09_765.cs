...
public class GridManager : MonoBehaviour
{
    ...
    private void ApplySprite( int x, int y, Piece piece )
    {
        ...
        renderer.sharedMaterial.SetTexture( "_MainTex", _mainImage );
        ...
        renderer.sharedMaterial.SetVector( "_Offset", offset );
        renderer.sharedMaterial.SetVector( "_Tiling", tiling );
        //pBlock.SetVector( "_MainTex_ST", new Vector4( tiling.x, tiling.y, offset.x, offset.y ) );

        renderer.sharedMaterial.SetTexture( "_Mask", _center );
        //if ( x == 0 && y == 0 )
        //{
