...
public class GridManager : NetworkBehaviour
{
    ...
    private Dictionary<TileType, Texture2D> _maskTiles = new();
    ...
    {
        _maskTiles.Add( TileType.TL, _topLeftCorner );
        _maskTiles.Add( TileType.TR,_topRightCorner );
        _maskTiles.Add( TileType.BL,_bottomLeftCorner );
        _maskTiles.Add( TileType.BR,_bottomRightCorner );

        _maskTiles.Add( TileType.B,_bottom );
        _maskTiles.Add( TileType.T,_top );
        _maskTiles.Add( TileType.L,_left );
        _maskTiles.Add( TileType.R, _right );
        _maskTiles.Add( TileType.C, _center);
    }
    ...
    private void Do()
    {
        ...
        for ( int y = 0; y < _height; y++ )
        {
            for ( int x = 0; x < _length; x++ )
            {
                ...
                pieceComponent.MaskTiles = _maskTiles;
    ...
    private void ApplySprite( int x, int y, Piece piece )
    {
        ...
        piece.TileType = TileType.C;

        if ( x == 0 && y == 0 )
            piece.TileType = TileType.BL;

        if ( x == 0 && y > 0 && y < _height - 1 )
            piece.TileType = TileType.L;

        if ( x == 0 && y == _height - 1 )
            piece.TileType = TileType.TL;

        if ( x > 0 && x < _length - 1 && y == _height - 1 )
            piece.TileType = TileType.T;

        if ( x == _length - 1 && y == _height - 1 )
            piece.TileType = TileType.TL;

        if ( x == _length - 1 && y > 0 && y < _height - 1 )
            piece.TileType = TileType.R;

        if ( x == _length - 1 && y == 0 )
            piece.TileType = TileType.BR;

        if ( x > 0 && x < _length - 1 && y == 0 )
            piece.TileType = TileType.B;
