...
public class GridManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _spawner.OnSceneLoadDone += OnSceneLoad;
    }

    private void OnSceneLoad( NetworkRunner runner )
    {
        StartCoroutine( Do() );
    }
    ...
    {
        if ( !_spawner.Runner.IsServer ) yield break;
        ...
        for ( int y = 0; y < _height; y++ )
        {
            for ( int x = 0; x < _length; x++ )
            {
                ...
                var piece = _spawner.Runner.Spawn( _piece, position, Quaternion.identity, null  );
                

                ApplySprite( x, y, piece.GetComponent<Piece>() );
