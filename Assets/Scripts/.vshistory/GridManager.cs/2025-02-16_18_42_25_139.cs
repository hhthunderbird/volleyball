...
public class GridManager : NetworkBehaviour
{
    ...
    private NetworkVariable<int> _height;
    private NetworkVariable<int> _width;
    ...
    public override void OnNetworkSpawn()
    {
        ...
        _mainImage = TextureManager.Instance.Texture;
    ...
    {
        var horizontalSize = _mainImage.width / ( float ) _width.Value;

        _height.Value = ( int ) ( _mainImage.height / horizontalSize );
        var finalVerticalSize = _mainImage.height / ( float ) _height.Value;
        ...
        var xScale = scale.x * _width.Value * 0.5f;
        var yScale = scale.y * _height.Value * 0.5f;
        ...
        bg.GetComponent<Renderer>().sharedMaterial.SetTexture( "_BaseMap", _mainImage );
        bg.position = _center;
    ...
    {
        var horizontalStart = ( _width.Value * scale.x * 0.25f ) - ( scale.x * 0.25f );
        var verticalStart = ( _height.Value * scale.y * 0.25f ) - ( scale.y * 0.25f );
        var start = new Vector3( horizontalStart, verticalStart, 0 );
        var position = new Vector3( x * scale.x * 0.5f, y * scale.y * 0.5f, 0 );
        position -= start;
        _center += position / ( _width.Value * _height.Value );

        var hOffsetStart = 1f / _width.Value / -2f;
        var vOffsetStart = 1f / _height.Value / -2f;

        var hOffset = 1f / _width.Value;
        var vOffset = 1f / _height.Value;

        var vTiling = 1f / _height.Value * 2;
        var hTiling = 1f / _width.Value * 2;
