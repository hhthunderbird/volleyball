...
public class GridManager : NetworkBehaviour
{
    ...
    private void InstantiateBlock(int x, int y )
    {
        ...
        if ( x == 0 && y == 0 )
            Runner.Spawn( _bl, position, Quaternion.identity, null );

        if ( x == 0 && y > 0 && y < _height - 1 )
            Runner.Spawn( _l, position, Quaternion.identity, null );

        if ( x == 0 && y == _height - 1 )
            Runner.Spawn( _tl, position, Quaternion.identity, null );

        if ( x > 0 && x < _length - 1 && y == _height - 1 )
            Runner.Spawn( _t, position, Quaternion.identity, null );

        if ( x == _length - 1 && y == _height - 1 )
            Runner.Spawn( _tr, position, Quaternion.identity, null );

        if ( x == _length - 1 && y > 0 && y < _height - 1 )
            Runner.Spawn( _r, position, Quaternion.identity, null );

        if ( x == _length - 1 && y == 0 )
            Runner.Spawn( _br, position, Quaternion.identity, null );

        if ( x > 0 && x < _length - 1 && y == 0 )
            Runner.Spawn( _b, position, Quaternion.identity, null );

        if ( x > 0 && x < _length - 1 && y > 0 && y < _height - 1 )
            Runner.Spawn( _c, position, Quaternion.identity, null );
