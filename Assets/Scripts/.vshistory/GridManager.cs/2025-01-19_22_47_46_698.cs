...
public class GridManager : MonoBehaviour
{
    ...
    private void ApplySprite( int x, int y, Piece piece )
    {
        ...
        GetComponent<Renderer>().sharedMaterial.SetVector( "_Offset", offset );
        GetComponent<Renderer>().sharedMaterial.SetVector( "_Tiling", tiling );
        //pBlock.SetVector( "_MainTex_ST", new Vector4( tiling.x, tiling.y, offset.x, offset.y ) );

        GetComponent<Renderer>().sharedMaterial.SetTexture( "_Mask", _center );
        //if ( x == 0 && y == 0 )
        //{
