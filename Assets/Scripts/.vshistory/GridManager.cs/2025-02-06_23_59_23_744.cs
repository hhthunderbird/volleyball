...
public class GridManager : NetworkBehaviour
{
    ...
    {
        Debug.LogWarning( $"Is session owner: {IsSessionOwner}" );
        Debug.LogWarning( $"object id: {this.NetworkObjectId}" );
        Debug.LogWarning( $"networkbehaviour id: {this.NetworkBehaviourId}" );
        Debug.LogWarning( $">>>>>>>> {NetworkManager.Singleton.CurrentSessionOwner == this.}" );

        if ( IsSessionOwner && NetworkManager.Singleton. )
            BuildGrid();
    }
