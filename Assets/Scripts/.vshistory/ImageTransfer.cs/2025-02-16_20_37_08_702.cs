...
public class ImageTransfer : NetworkBehaviour
{
    ...
    public event Action OnReceivedImage;

    public void SendImageToPeers( byte[] imageData )
    {
        Debug.Log( "seding image" );
        ...
        for ( int i = 0; i < totalChunks; i++ )
        {
            ...
            SendChunkToAuthorityRpc( chunk, i, totalChunks );
        }
    }

    [Rpc(SendTo.Authority)]
    private void SendChunkToAuthorityRpc(byte[] chunk, int chunkIndex, int totalChunks, RpcParams rpcParams = default)
    {
