...
public class ImageTransfer : NetworkBehaviour
{
    ...
    public Texture2D SelectedTexture => _selectedTexture;
    ...
    private void ReconstructImage()
    {
        ...
        _selectedTexture = new Texture2D( 2, 2 );
        _selectedTexture.LoadImage( imageData );

        OnReceivedImage?.Invoke( _selectedTexture );

        TextureManager.Instance.Textures.Add( _selectedTexture );
