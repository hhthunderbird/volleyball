...
public class DragAndDropManager : NetworkBehaviour
{
    ...
    public void Update()
    {
        ...
        {
            var screenPos = _input.InputPosition.Value;// Input.mousePosition;
            ...
            Debug.Log( $"{_currentObject.transform.position}   {_input.InputPosition}" );
        }

        if ( _currentObject == null && _input.Grabbed.Value )
        {
            //Debug.Log( "GRABBED" );
            Grab();
        }

        if ( _currentObject != null && !_input.Grabbed )
        {
            //Debug.Log( "RELEASED" );
            Release();
        }
