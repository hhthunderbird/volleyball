...
public class DragAndDropManager : NetworkBehaviour
{
    ...
    private void Check()
    {
        Debug.Log( "çalkdsfçalkdjs" );
        var ray = Camera.main.ScreenPointToRay( Input.mousePosition );
        if(Physics.Raycast( ray.origin, ray.direction, out var hitInfo, 1000, LayerMask.NameToLayer("piece") ) )
        {
            var piece = hitInfo.transform.GetComponent<Piece>();

            Debug.Log( $"state auth {piece.HasStateAuthority}" );
