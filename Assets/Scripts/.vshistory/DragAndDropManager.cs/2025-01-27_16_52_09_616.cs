...
public class DragAndDropManager : NetworkBehaviour
{
    ...
    [SerializeField] private bool _isInputDown;
    [SerializeField] private bool _isInputUp;

    private void Update()
    {
        _isInputDown = Input.GetMouseButtonDown( 0 );
        _isInputUp = Input.GetMouseButtonUp( 0 );
    }
    ...
    public override void FixedUpdateNetwork()
    {
        ...
        if ( input.Value.IsInputPressed )
            Check();

        if ( input.Value.IsInputReleased )
            Drop();
