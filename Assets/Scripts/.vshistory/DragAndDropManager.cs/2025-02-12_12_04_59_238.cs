...
public class DragAndDropManager : NetworkBehaviour
{
    ...
    [SerializeField] private bool _clickState;
    ...
    public void Update()
    {
        if ( !IsOwner ) return;
            ...
            Debug.Log( $"xxxx {_currentObject != null} && {_currentObject.IsOwner} && {_isDragging}" );
        if ( _currentObject != null && _currentObject.IsOwner && _isDragging)
        {
