...
public class BallBehaviour : NetworkBehaviour
{
    ...
    private Vector3 Target( Vector3 launchDirection, float velocity, float angle, float initialHeight )
    {
        ...
        var xComponent = velocity * Mathf.Cos( angle ); // Horizontal velocity in x-z plane
        var yComponent = velocity * Mathf.Sin( angle ); // Vertical velocity

        // Formula for range with initial height
        float x = ( xComponent / gravity ) * ( yComponent + Mathf.Sqrt( Mathf.Pow( yComponent, 2 ) + 2 * gravity * initialHeight ) );
