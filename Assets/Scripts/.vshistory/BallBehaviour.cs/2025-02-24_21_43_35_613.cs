...
public class BallBehaviour : NetworkBehaviour
{
    ...
    private Vector3 Target( Vector3 launchDirection, float velocity, float angle, float initialHeight )
    {
        ...
        float zDistance = xDistance * ( launchDirection.z / launchDirection.x ); // Proportional to the x component

        // Calculate the target position at ground height (y = 0)
        Vector3 groundTarget = transform.position + new Vector3( xDistance, 0, zDistance );
