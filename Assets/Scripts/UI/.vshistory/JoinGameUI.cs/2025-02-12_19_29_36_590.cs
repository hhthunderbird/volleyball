...
public class JoinGameUI : MonoBehaviour
{
    ...
    private UniTask _searchRoomTask;
    ...
    private void SetLobbyName( string sessionName )
    {
        _confirmButton.interactable = !string.IsNullOrEmpty( sessionName );
        //ClientInfo.LobbyName = lobby;

        _searchRoomTask = UniTask.Run( () =>
        {
            try
            {
        ...
        }, true, _cancellationTokenSource );

    }

    public async UniTask<bool> CheckIfSessionExistsByName( string sessionName )
    {
        try
        {
            // Query lobbies with the specified name
