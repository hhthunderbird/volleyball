...
public class JoinGameUI : MonoBehaviour
{
    ...
    public UniTaskVoid CheckIfSessionExistsByName( string sessionName, CancellationToken cancellationTokenSource )
    {
        ...
        try
        {
            ...
            // Query the lobbies
            var lobbyList = await LobbyService.Instance.QueryLobbiesAsync( query, cancellationToken: cancellationToken );

            // Process the results
            Debug.Log( $"Found {lobbyList.Results.Count} lobbies matching '{searchQuery}'" );
