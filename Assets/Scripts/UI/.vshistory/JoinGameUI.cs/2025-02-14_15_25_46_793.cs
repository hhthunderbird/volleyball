...
public class JoinGameUI : MonoBehaviour
{
    ...
    private async void OnConfirm()
    {
        try
        {
            ...
            await MultiplayerService.Instance.CreateOrJoinSessionAsync( _lobbyName, options );
    ...
    public async UniTaskVoid CheckIfSessionExistsByName( string sessionName, CancellationToken cancellationTokenSource )
    {
        try
        {
            ...
            if ( lobbyList.Results.Count > 0 )
            {
                ...
                Debug.Log( $"Lobby Name: {lobby.Name}, Available slots: {lobby.AvailableSlots} code {lobby.LobbyCode}" );
            }
